...
//            cv_bridge::CvImageConstPtr ptr = cv_bridge::toCvCopy(, sensor_msgs::image_encodings::MONO8);
    ...
    for (int i = 0; i < NUM_OF_CAM; i++)
    {
        ...
        if (i != 1 || !STEREO_TRACK)
            trackerData[i].readImage(image.rowRange(ROW * i, ROW * (i + 1)));
        else
        {
            if (EQUALIZE)
            {
                cv::Ptr<cv::CLAHE> clahe = cv::createCLAHE();
                clahe->apply(image.rowRange(ROW * i, ROW * (i + 1)), trackerData[i].cur_img);
            }
            else
                trackerData[i].cur_img = image.rowRange(ROW * i, ROW * (i + 1));
        }
...
void process()
{
    ...
    while (true)
    {
        ...
        {
            auto feature_points=trackImage(measurement.second);
