...
//    cv_bridge::CvImagePtr img_ptr = cv_bridge::toCvCopy(img_msg, sensor_msgs::image_encodings::MONO8);
    image_pool[img_msg->header.stamp.toNSec()] = cv::Mat(img_msg->height,img_msg->width,CV_8UC1,(void*)img_msg->data.data());
    auto feature_tracked=trackImage(img_msg);
    if(feature_tracked){
        image_callback(feature_tracked);
    }
}
